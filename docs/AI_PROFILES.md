...
Balanced is the active default in ruleset `prism-authorized-baseline@2.0.0`. ExitPolicyV2 is ruleset-owned: it arms at +20%, trails 10 percentage points from MFE, takes hard profit at +40%, and retains a -50% hard stop. Final executable sizing is always the minimum of target allocation, per-trade stop-risk, ticker/sector/cluster/portfolio caps, regime, liquidity, and buying-power constraints.
...
Post-Analysis may recommend only the two fields above. Exit-policy calibration remains manual until ShadowFund has complete policy-comparison evidence. The authenticated operator selects `manual` or `automatic` calibration through the backend profile-governance API; a missing preference is returned as a non-persistent manual default. Manual activation requires an explicit authenticated `POST /api/v1/profiles/activate-post-analysis` request for a complete draft batch. Neither mode can change an immutable ruleset or any hard control.
