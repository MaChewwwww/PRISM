...
## Day 1 option-data behavior

Option-chain and option-quote requests explicitly use Alpaca Indicative data. The official historical option-bars endpoint has no selectable feed parameter; the gateway never claims an Indicative option-bars feed. If that historical request is unavailable or returns no usable bars, it is safely surfaced as unavailable and the current reduced-history IV resolution continues from durable observations, configured-provider observations, and the current option-chain observation. Monitoring exposes the configured and effective observation counts, resolution path, and fallback status without changing authorization behavior.
