...
| Autonomous paper-trading schedule | `AUTONOMOUS_TRADING_ENABLED`, `AUTONOMOUS_TRADING_START_AT`, `AUTONOMOUS_TRADING_END_AT` | No AI or operator shortcut; future orchestration remains behind deterministic validation | `Settings.autonomous_trading_window_active`; production bound to BA window; staging rehearsal remains paper-only | execution configuration and future scheduler | settings, window-boundary, paper-only, and no-invocation tests |
