...
| Autonomous paper-trading schedule | Production-only `AUTONOMOUS_TRADING_ENABLED`, `AUTONOMOUS_TRADING_START_AT`, `AUTONOMOUS_TRADING_END_AT`, 900-second cadence, seven-symbol allowlist, six-position cap | No AI or operator shortcut; production worker records fail-closed cycle outcomes; staging configuration rejects the flag | `AutonomousWorker`, `autonomous_controls`, `autonomous_cycles`, authenticated control endpoints | production execution configuration and status | settings, advisory-lock, readiness, kill-switch, staging-rejection, and no-invocation tests |
| Staging historical backtest | `BACKTEST_SIMULATION_ENABLED`, fixed historical window, immutable input/output artifacts | No order adapter, active portfolio, or autonomous-worker path may be used; incomplete historical evidence is `NO_TRADE`/`DATA_UNAVAILABLE` | `app.backtest`, `backtest_runs`, `backtest_audit_events` | planned staging presentation projection selects only a successfully completed simulated run; data-refusal runs remain inactive | staging gate, no-execution-import, artifact, projection, and provenance tests |
