...
function toPath(points: OverviewPoint[], key: "actual" | "alt" | "bench", scale: ReturnType<typeof buildScale>) {
        ...
        <div className="overview-chart-body">
          <div className="overview-chart-plot">
            <svg
              ...
              role="img"
              aria-label="Portfolio, alternative, and benchmark equity paths over the selected date range, with the gap between the portfolio and the best alternative shaded"
            >
              ...
              <polyline points={benchPath} className="overview-chart-line overview-chart-line-bench" />
              <polyline points={altPath} className="overview-chart-line overview-chart-line-alt" />
              <polyline points={actualPath} className="overview-chart-line overview-chart-line-actual" />
            </svg>
              ...
              {points.map((point, index) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`${point.date} at ${point.time}: portfolio ${formatCurrency(point.actual)}${
            ...
            {activePoint && activeIndex !== null && (
              <div
                ...
                style={{
                  ...
                  ...(scale.xFor(activeIndex) > 70 ? { transform: "translateX(-100%)" } : undefined),
                }}
                role="status"
              >
                <strong>
  ...
  return (
    <aside className="overview-chart-detail" aria-label="Selected point detail">
      <div className="overview-chart-detail-head">
        ...
        <button type="button" className="overview-chart-detail-close" onClick={onClose} aria-label="Close chart detail">
      ...
      {point.decision ? (
        <div className="overview-chart-detail-decision">
          ...
          <dl>
            ...
            <div>
              <dt>Paper result</dt>
              <dd className="overview-nums">{formatSignedCurrency(point.decision.paper)}</dd>
            </div>
          </dl>
          <Link className="text-link" href={point.decision.storyId ? `/stories/${point.decision.storyId}` : "/stories"}>
            View decision story
          </Link>
        </div>
      ) : (
